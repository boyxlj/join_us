<template>
  <div class="positionCard" v-if="data.length">
    ...
    <div class="box">
      <div
        class="item"
        @click="navigateDetails(item.position_id)"
        v-for="item in data"
        :key="item.id"
      >
        <li class="top">
          <span class="name" :title="item.position_name">{{
            item.position_name
          }}</span>
        ...
        <li class="bottom">
          ...
          <span class="city"
            >{{ item.cityName }} <b v-if="item.region"></b
            >{{ item.region }}</span
          >
        </li>
      </div>
...
const props = withDefaults(
  defineProps<{ origin?: string; title?: string; dataList?: any }>(),
  {
    title: '推荐职位',
    dataList: [],
