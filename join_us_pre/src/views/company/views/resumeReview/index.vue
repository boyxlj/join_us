    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'position_state'">
          <a-tag v-show="record.position_state === '1'" color="success"
            >正常</a-tag
          >
          <a-tag v-show="record.position_state === '0'" color="warning"
            >待审核</a-tag
          >
          <a-tag v-show="record.position_state === '2'" color="error"
            >已关闭</a-tag
          >
        </template>
        ...
        <template v-if="column.key === 'oprate'">
          <a-button
            ...
            style="margin: 0 5px"
            type="primary"
            @click="openModal(record.userId)"
            >查看简历</a-button
          >
          <a-button
            size="small"
            :disabled="record.types === '1' || record.position_state !== '1'"
            style="margin: 0 5px"
            type="primary"
            @click="updateState(record.sendId, 'interview')"
            >约面试</a-button
          >
          <a-button
            size="small"
            :disabled="record.types === '2' || record.position_state !== '1'"
            style="margin: 0 5px"
            type="danger"
            @click="updateState(record.sendId, 'inappropriate')"
            >不合适</a-button
          >
        </template>
      </template>
